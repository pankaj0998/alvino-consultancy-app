    ...
    return (
        <div className="w-full rounded-lg overflow-hidden shadow-lg max-h-max">
            ...
            <div className={`max-[600px]:p-4 bg-gray-custom ${cardData && description ? 'p-6' : 'px-6 pt-6 pb-0'}`}>
                <h2 className="text-xl max-[600px]:text-[16px] font-bold text-blue-title mb-2">{title}</h2>

                {/* Optional Description */}
                {description && (
                    <p className="text-gray-description mb-4 max-[600px]:text-[12px]">{description}</p>
                )}
